...
- dcde8ab: BREAKING: `FallbackBindingConfig.snapshotSource` is now required. Declare `'endpoint'` when the snapshot is a route the server answers, `'synthesized'` when the transport answers it locally (the push-only pattern).
