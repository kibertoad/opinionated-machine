  ...
  async publish(
    ...
    if (this.presence) {
      try {
        ...
        // Fail-open. Surface the error via the optional observability hook so
        // operators can detect a silently-broken tracker. The hook itself must
        // not disrupt publishing.
