...
import type { SSEConnection, SSEMessage } from '../sse/AbstractSSEController.ts'
import type { AnySSERouteDefinition, SSERouteDefinition } from '../sse/sseContracts.ts'

/**
 * Predicate function to match connections.
 */
export type ConnectionPredicate = (connection: SSEConnection) => boolean

/**
 * Options for waiting for a connection.
 */
export type WaitForConnectionOptions = {
  /** Timeout in milliseconds (default: 5000) */
  timeout?: number
  /** Optional predicate to match specific connections */
  predicate?: ConnectionPredicate
}

/**
 * Pending connection waiter with its resolver.
 */
...
/**
 * A spy for SSE connections that handles the race condition where connections
 * may arrive before or after we start waiting for them.
 *
 * Similar to HandlerSpy from message-queue-toolkit, this uses a dual-buffer
 * strategy:
 * 1. A buffer of established connections (for late-arriving queries)
 * 2. A registry of pending promises (for connections that arrive after we start waiting)
 *
 * @example
 * ```typescript
 * // Create spy and attach to controller
 * const spy = new ConnectionSpy()
 * controller.onConnectionEstablished = (conn) => spy.addConnection(conn)
 * controller.onConnectionClosed = (conn) => spy.removeConnection(conn.id)
 *
 * // Start connection (async, may complete before or after waitForConnection)
 * connectSSE(baseUrl, '/api/stream')
 *
 * // Wait for connection - resolves immediately if already connected,
 * // or waits until connection arrives
 * const connection = await spy.waitForConnection()
 *
 * // Wait for specific connection
 * const userConn = await spy.waitForConnection({
 *   predicate: (c) => c.context.userId === 'user-123'
 * })
 * ```
 */
export class ConnectionSpy {
  ...
  /**
   * Add a connection to the spy.
   * If there are pending waiters that match this connection, they are resolved.
   *
  ...
  addConnection(connection: SSEConnection): void {
    ...
    // Check if any waiters match this connection
    const matchingWaiterIndex = this.waiters.findIndex((w) => w.predicate(connection))
    if (matchingWaiterIndex !== -1) {
      const waiter = this.waiters[matchingWaiterIndex] as ConnectionWaiter
      clearTimeout(waiter.timeoutId)
      this.waiters.splice(matchingWaiterIndex, 1)
      waiter.resolve(connection)
    }
  }

  /**
   * Remove a connection from the spy.
   *
   ...
   * If a matching connection already exists, resolves immediately.
   * Otherwise, waits until a matching connection arrives or timeout.
   *
   * @param options - Wait options (timeout, predicate)
   * @returns The matching connection
   * @throws Error if timeout is reached before a matching connection arrives
   */
  waitForConnection(options?: WaitForConnectionOptions): Promise<SSEConnection> {
   ...
   *
   * @param count - Number of connections to wait for
   * @param options - Wait options (timeout, predicate)
  ...
  async waitForConnections(
    ...
    // Collect already-matching connections
    for (const connection of this.connections.values()) {
      if (predicate(connection) && connections.length < count) {
    ...
    while (connections.length < count) {
      ...
      if (remaining <= 0) {
        throw new Error(
          `Timeout waiting for ${count} connections after ${timeout}ms (got ${connections.length})`,
        )
      }

      const conn = await this.waitForConnection({
        timeout: remaining,
        predicate: (c) => predicate(c) && !connections.some((existing) => existing.id === c.id),
      })
  ...
  /**
   * Clear all connections and cancel pending waiters.
   */
  clear(): void {
