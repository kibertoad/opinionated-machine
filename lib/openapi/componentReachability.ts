 ...
 *
 * Used both by `stripInternalOperations` (after it removes internal
 * operations) and, on its own, as a `transformObject` step —
 * `fastify-type-provider-zod`'s `jsonSchemaTransformObject` emits *every*
 * schema in the Zod registry into `components.schemas`, with no knowledge of
 * which routes survived the audience transform, so the customer-facing
 * document otherwise carries internal request and response shapes.
 */
 ...
 *
 * The companion to `openApiVisibilityTransform` when schemas reach
 * `components` rather than being inlined — which is what
 * `fastify-type-provider-zod`'s `jsonSchemaTransformObject` does. That
 * function writes the whole Zod registry into `components.schemas`
 * unconditionally: it runs once over the finished document and never sees
 * which operations the transform hid, so an internal-only response shape
 * lands in the public document even though no public operation references it.
 *
 * Chain this underneath it to bring the document back in line with its own
 * operation set:
 *
 * @example
 * ```ts
