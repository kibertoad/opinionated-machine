...
Bound fallback connects and polls, clamp `retry:` hints, and recover from hydration and parsing failures.
...
- **CRLF-framed streams parse correctly** in the vendored client parser: the blank separator line kept a trailing `\r`, so consecutive events merged into one with the wrong id and concatenated data.
